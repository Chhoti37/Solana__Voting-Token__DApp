...
mod events;
use events::*;
declare_id!("DE5W91snEaUZV6jNC4qWGLAhyzewxdapoTbHZ3pDb6Gz");

// --- FIX 1: Explicit Imports ---
use anchor_spl::token::{mint_to, transfer, MintTo, Transfer}; // 'token' module import kiya taaki conflict na ho
...
pub mod vote_app {
    ...
    pub fn initialize_treasury(ctx: Context<InitializeTreasury>, sol_price: u64, tokens_per_purchase: u64) -> Result<()> {
        ...
        require!(proposal_counter_account.proposal_count == 0, VoteError::ProposalCounterAlreadyInitialized); // Just to initialize proposal count to 0
    ...
    pub fn buy_tokens(ctx: Context<BuyTokens>) -> Result<()> {
        ...
        // Type inference (let cpi_ctx = ...) use kiya hai complexity kam karne ke liye
    ...
    pub fn register_proposal(ctx: Context<RegisterProposal>,proposal_info: String,deadline: i64,token_amount: u64) -> Result<()> {
        ...
        //transfer tokens from token account to tresury token account
    ...
    pub fn proposal_to_vote(ctx: Context<Vote>,proposal_id:u8,token_amount:u64) -> Result<()> {
        ...
        proposal_account.number_of_votes = proposal_account.number_of_votes.checked_add(1).ok_or(VoteError::ProposalCounterOverflow)?;
    ...
    pub fn pick_winner(ctx: Context<PickWinner>,proposal_id:u8) -> Result<()> {
        ...
        // update winner if this proposal has more votes than current winner
        if proposal.number_of_votes > winner.winning_votes {
            ...
            winner.declared = clock.unix_timestamp;
    ...
    pub fn withdraw_sol(ctx: Context<WithdrawSol>, amount: u64) -> Result<()> {
        ...
        //Use signing to transfer SOL from vault to authority
